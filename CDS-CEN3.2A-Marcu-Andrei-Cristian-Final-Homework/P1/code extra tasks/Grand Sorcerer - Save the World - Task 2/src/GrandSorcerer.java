...
public class GrandSorcerer extends Thread{
	...
	ConcurrentHashMap<Integer, Integer> sleepingDemonsNumber; // the number of sleeping demons in each coven
	...
	GrandSorcerer()
	{
		sleepingDemonsNumber = new ConcurrentHashMap<Integer, Integer>();
		rand = new Random();
		...
		for(int i = 0 ; i < covensNo; i++)
		{
			sleepingDemonsNumber.put(i, 0); // we initialize the number of sleeping demons in each coven with 0
		}
