...
public class Demon extends Thread {
	...
	public void run() {
		while(isRetired == false && coven.grandSorcerer.gameEnded == false) // while demon is not retired and undeads aren't defeated
		{
		 ...
		 // if we can acquire the semaphore used by all demons that means we are the random demon that is going to retire
		if(coven.grandSorcerer.grandSorcererHelper.demonRetireSemaphore.tryAcquire() && coven.grandSorcerer.gameEnded == false) {
			System.out.println("Demon " + demonID + " was retired by the demon retire thread");
			retireDemon();
		}
		// we have only one instance of grandSorcerer and respective grandSorcererHelper so the semaphore will be common for all instances of demon
	}
